...
import { Home, ArrowLeftRight, TrendingUp, Users, Briefcase, LogOut, Info } from 'lucide-react';
...
// 6. Google Login Actions
  ...
  const handleOpenAddTransaction = (playerId?: string) => {
  ...
  const saveOwnerPrivateData = async (data: { accounts: any[]; transactions: any[] }, passcode: string) => {
    if (!user) return;
...
/>
          ...
          <div className="space-y-4">
            <button
              onClick={handleLogin}
              className="w-full flex items-center justify-center gap-3 px-5 py-3.5 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-semibold shadow-lg hover:shadow-slate-900/10 transition-all duration-200 cursor-pointer active:scale-98 group"
            >
          ...
          <div className="absolute bottom-0 left-0 right-0 border-t border-slate-200/50 bg-gradient-to-t from-slate-50/50 to-transparent p-4">
            <p className="text-[11px] text-slate-400 font-medium">Developed by</p>
            <p className="text-xs text-slate-500 font-semibold mt-0.5">Ye Htun Naing ✦</p>
            <p className="text-[10px] text-slate-300 mt-2">© 2026 Player Finance</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-850 flex flex-col font-sans select-none antialiased">
      {/* Top Banner Accent */}
      <div className="h-1.5 w-full bg-blue-600 fixed top-0 left-0 z-50 shadow-xs" />
